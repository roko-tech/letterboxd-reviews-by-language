...
(function () {
  ...
  // runs in an isolated world, so reach them via Tampermonkey's unsafeWindow.
   ...
   * Allowed languages
   * ------------------------------------------------------------------ */
  let languages = GM_getValue('languages', ['ar']); // BCP-47 base codes; Arabic by default
      ...
      try {
        ...
        if (status !== 'available' && !viaUserGesture) {
          ...
          if (!ensureDetector._menu) {
            ...
            GM_registerMenuCommand('⬇️ تنزيل نموذج اللغة الدقيق', () => ensureDetector(true));
          }
          return null;
        }
  ...
  function openPanel() {
    ...
    panel.innerHTML =
      '<div id="llc-panel-bar">' +
        '<span id="llc-panel-title">المراجعات العربية</span>' +
        '<select id="llc-sort" title="ترتيب المراجعات">' +
          '<option value="default">ترتيب Letterboxd</option>' +
          '<option value="date-desc">الأحدث</option>' +
          '<option value="date-asc">الأقدم</option>' +
          '<option value="rating-desc">الأعلى تقييمًا</option>' +
          '<option value="rating-asc">الأقل تقييمًا</option>' +
          '<option value="len-desc">الأطول</option>' +
          '<option value="len-asc">الأقصر</option>' +
        '</select>' +
        '<span id="llc-panel-status"></span>' +
        '<button id="llc-panel-stop">إيقاف</button>' +
        '<button id="llc-panel-close">✕ إغلاق</button>' +
      '</div><div id="llc-panel-list"></div>';
    document.body.appendChild(panel);
    const list = panel.querySelector('#llc-panel-list');
    // RTL reading for Arabic/Hebrew — scoped to the review text via a class so
    // the header (name/date/rating) stays LTR and dates don't reorder.
  ...
  function updateCollectBtn() {
    if (collectBtn) collectBtn.textContent = '📚 اجمع المراجعات العربية';
  }
  function ensureCollectBtn() {
    if (collectBtn || !filmSlug()) return;
    collectBtn = document.createElement('div');
    collectBtn.id = 'llc-collect';
    collectBtn.title = 'اجمع كل المراجعات العربية من جميع الصفحات';
  ...
  async function collectAllReviews() {
    ...
    const slug = filmSlug();
    if (!slug) { alert('افتح صفحة فيلم على Letterboxd أولًا.'); return; }

    collecting = true;
    if (collectBtn) collectBtn.textContent = '📚 جارٍ الجمع…';
    const ui = openPanel();
    ...
    try {
      ...
      while (!done && !stopped && next <= HARD_CAP) {
        ...
        for (const pg of fetched) {
          ...
          // when the user picks an option (the 'change' listener below).
          if (panel) ui.status.textContent = `فُحصت ${scanned} صفحة … ${found} نتيجة`;
    ...
    if (panel) {
      ...
      ui.status.textContent = rateLimited
        ? `⚠️ حدّ Letterboxd من الطلبات — جُمعت ${found} مراجعة حتى الآن (فُحصت ${scanned} صفحة، غالبًا غير مكتمل). انتظر دقيقة ثم أعد المحاولة.`
        : `${stopped ? 'أُوقف' : 'اكتمل'} — ${found} مراجعة (فُحصت ${scanned} صفحة).`;
      if (!found) ui.list.innerHTML = '<p style="color:#9ab;padding:24px" dir="rtl">لا توجد مراجعات بهذه اللغة.</p>';
    }
  }
  ...
  function registerMenu() {
    GM_registerMenuCommand('🌐 تحديد اللغات…', () => {
      const input = prompt(
        'اجمع هذه اللغات فقط (رموز BCP-47 مفصولة بفواصل).\n' +
        'أمثلة: ar  |  en  |  en, ar  |  en, es, fr',
        languages.join(', ')
      );
    ...
    GM_registerMenuCommand('📚 جمع المراجعات (كل الصفحات)', collectAllReviews);
  }
  ...
  GM_addStyle(`
    ...
    #llc-panel-bar {
      direction: rtl; /* Arabic UI chrome reads right-to-left */
    ...
    /* Full RTL cards so rating/name/avatar sit on the right, next to the Arabic text. */
